...
Связи:
* Ассортимент (“Product line”).
  Просматривается покупателем.
* Заказ (“Order”).
...
Связи:
* Ассортимент (“Product”).  
  Формируется/редактируется работником отдела продаж.
* Заказ (“Order”). 
...
* Работник отдела продаж ("Salesman"). 
  Формирует/редактирует ассортимент.
* Покупатель (“Customer”).
  Изучает ассортимент.
...
* Пивовар ("Brewer"). 
  По завершени производства изменяет количество пива в базе. 
...
```
POST /brewery/customer/products
```
```json
[
  {
